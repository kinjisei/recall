 ...
 *
 * ⚠️ Тест обязан КРАСНЕТЬ, если снять ограничение. Проверено подделками, а не
 * обещано: окно покрытия, сведе́ние словарных пунктов, починка баланса, обрезка
 * набора, роспуск группы из одного пункта — каждая роняет свой блок.
 *
 * ⚠️ Числа правил сверяются с ЛИТЕРАЛАМИ, а не с импортированной константой.
 * Сравнение вида `budget <= d * NEW_WORDS_PER_DAY_MAX` упасть не может: граница
 * едет вместе с константой. Так и было — дневной потолок поднимался с 10 до 40,
 * а набор пунктов с 6 до 12, и тест оставался зелёным 55/55. Соседние тесты
 * (test-word-packs, test-material-exercises) прибивают числа литералами; здесь
 * этого не хватало.
 *
...
// ---------------------------------------------------------------------------
console.log('\n— числа правил не уплыли —')

// Числа взяты из исследований (заход «Собрать домашку»), а не подобраны на
// глаз, поэтому менять их — продуктовое решение, а не рефакторинг. Литералы
// здесь и нужны, чтобы такое решение нельзя было принять молча.
check('новых слов в день — не больше 10', NEW_WORDS_PER_DAY_MAX === 10, `${NEW_WORDS_PER_DAY_MAX}`)
check('за заход — 8 новых слов', NEW_WORDS_PER_SESSION === 8, `${NEW_WORDS_PER_SESSION}`)
check('коротких заходов в неделю — 4', SESSIONS_PER_WEEK === 4, `${SESSIONS_PER_WEEK}`)
check('повторение за заход — 20 карточек', REVIEW_PER_SESSION === 20, `${REVIEW_PER_SESSION}`)
check('в наборе не больше 6 пунктов', MAX_ITEMS === 6, `${MAX_ITEMS}`)
check('на выбор — ровно 2 пункта', PICK_SIZE === 2, `${PICK_SIZE}`)
check(
  'окно покрытия — 95-98%',
  COVERAGE_WINDOW.min === 0.95 && COVERAGE_WINDOW.max === 0.98,
  `${COVERAGE_WINDOW.min}-${COVERAGE_WINDOW.max}`,
)
...
// Главная проверка правила, выраженная НЕЗАВИСИМО от того, какая константа его
// держит: сколько бы слов ни выдал подбор, в пересчёте на день их не больше
// десяти. Сейчас связывает объём захода (8 слов ≈ 4,6 в день на неделе), а
// дневной потолок не срабатывает ни на одном сроке — он страховка на случай,
// если объём захода вырастет. Поднимите его до 12 — эта проверка покраснеет;
// прежняя, сверявшая бюджет с самим потолком, оставалась зелёной.
let worstRate = 0
let worstDay = 0
for (let d = 1; d <= 60; d++) {
  const rate = newWordsBudget(d) / d
  if (rate > worstRate) {
    worstRate = rate
    worstDay = d
  }
...
check(
  'в пересчёте на день — не больше 10 новых слов, на любом сроке',
  worstRate <= 10,
  `худший срок: ${worstDay} дн. → ${worstRate.toFixed(1)} слов/день`,
)

check('на неделю — 32 новых слова (8 за заход × 4 захода)', newWordsBudget(7) === 32, `${newWordsBudget(7)}`)
check(
