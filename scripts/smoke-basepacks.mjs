...
const EMAIL = 'base-smoke@recall.test'
  ...
  try {
    ...
    await page.evaluate(() => {
      ;[...document.querySelectorAll('button')]
        .find((b) => b.textContent.includes('Уровень A2'))?.click()
    })
    await new Promise((r) => setTimeout(r, 500))
    const topicOk = await page.evaluate(() => document.body.textContent.includes('База A2'))
    check('темы «База A2» видны', topicOk)

    // тема — строка списка с отдельной кнопкой «+ Добавить» рядом
    const added = await page.evaluate(() => {
      const rows = [...document.querySelectorAll('div, li')].filter(
        (el) => el.textContent.includes('База A2') && el.querySelector('button'),
      )
    ...
    check('пак «База A2» добавлен', added === 'ok', added)
    ...
    check('карточки базы появились в колоде', (count ?? 0) > 0, `карточек: ${count}`)
