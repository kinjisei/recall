...
- 🔶 **Дизайн-система (полировка всего проекта)** — токены/базовые стили в `index.css`,
  ...
  ⚠️ Визуально вживую НЕ проверено (нет headless-браузера + экраны за авторизацией) —
  проверялись только сборка и трансформ модулей. Прогнать `npm run dev` глазами.

Дальше (позже):
- Фаза 4 — режим «Преподаватель» (роли, назначение колод, прогресс учениц; ветка phase-4-teacher).
