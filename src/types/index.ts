...
export interface WritingGrade {
  ...
  /**
   * 2–3 ПОВТОРЯЮЩИХСЯ типа ошибок — то, над чем работать в этот раз.
   *
   * ⚠️ Разбор, размечающий весь текст, работает хуже направленного: список из
   * двадцати правок не даёт понять, что чинить первым, и вдобавок учит
   * избегать сложных конструкций — безопаснее написать проще, чем получить
   * очередную россыпь пометок. Поэтому сверху 2–3 повторяющихся типа, а полный
   * список остаётся, но за раскрывашкой.
   */
  focus?: { type: string; why: string; examples: { was: string; fix: string }[] }[]
