// ============================================================================
// Программа обучения по неделям: AI (tier max) составляет план под конкретного
// ученика — берёт уровень, цель преподавателя и СЛАБЫЕ МЕСТА из диагностики
...
export async function generateStudyPlan(
  ...
  const system = [
    ...
    `Составь программу обучения на ${req.weeks} недель(и) для ученика (${langName}, уровень ${req.level}).`,
    ...
    '4. Уровень тем — уровень ученика или на шаг выше, не прыгай через голову.',
  ...
  const userMsg = [
    ...
    '',
    'Диагностика ученика (реальные данные приложения):',
 ...
 * оставлял ученика вообще без активной программы (находка ревью 2026-07-24).
 */
