...
export const GEMINI_TIER_CHAINS: Record<AiTier, string[]> = {
  ...
  // частое (Диалог, письмо, квесты, разбор работ). ФАКТ от владельца (24.07,
  // AI Studio): у 3.6-flash, 3.5-flash и 2.5-flash ОДИНАКОВЫЕ лимиты —
  // RPM 5, RPD 20. Это ~60 запросов в сутки на всех пользователей вместе:
  // поток Диалога они не вывезут, а каждый запрос сверх лимита — лишний
  // 429 и задержка. Поэтому первой идёт gemma-4-31b (RPM 30, RPD 14 400) —
  // единственная модель, способная нести нагрузку; flash остаются
  // страховкой на случай сбоя gemma, терминальный резерв — Groq-70b.
  standard: [
    'gemma-4-31b-it',
    'gemini-3.6-flash',
    'gemini-3.5-flash',
